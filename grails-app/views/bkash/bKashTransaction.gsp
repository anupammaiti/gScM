...
<head>
    ...
    <title>Online Bkash Payment</title>
</head>
<body>
<grailslab:breadCrumbActions breadCrumbTitleText="Bkash Payment Summary" SHOW_PRINT_BTN="YES"/>
<div class="row">
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="col-md-6 no-padding"><a href="javascript:void(0)" referenceId="${dataSet.DT_RowId}"
                                                                         ...
                                                                         title="Verify"><span style="color: red"
                                                class="green glyphicon glyphicon-ok"></span></a></span>
...
<script>
    ...
    jQuery(function ($) {
        var $listTable = $('#list-table').dataTable({
            "sDom": "<'row'<'col-md-2 paymentStatusFilterHolder'><'col-md-2 startDate-filter-holder'><'col-md-3 endDate-filter-holder'><'col-md-2 className-filter-holder'><'col-md-3'f>r>t<'row'<'col-md-3'l><'col-md-3'i><'col-md-6'p>>",
            "bAutoWidth": true,
            "bServerSide": true,
            "iDisplayLength": 25,
            "aaSorting": [0,'desc'],
            ...
            "sAjaxSource": "${g.createLink(controller: 'bkash',action: 'listManage')}",
        ...
        $('#list-table_wrapper div.paymentStatusFilterHolder').html('<select id="paymentStatus" class="form-control" name="paymentStatus"><option value="verified">Verified</option><option value="draft">Not Verified</option></select>');
        ...
        $('#list-table').on('click', 'a.delete-reference', function (e) {
            ...
            var invoiceId = $(this).attr('referenceId');
            if (confirmDel == true) {
                window.location = "${g.createLink(controller: 'bkash',action: 'onlinePayment')}?invoiceId="+invoiceId;
            }
            e.preventDefault();
        });
    ...
    function getActionButtons(nRow, aData) {
        var actionButtons = "";
        actionButtons += '<span class="col-md-6 no-padding"><a href="javascript:void(0)" referenceId="' + aData.DT_RowId + '" class="delete-reference" title="Verify">';
        actionButtons += '<span style="color: red" class="green glyphicon glyphicon-ok"></span>';
