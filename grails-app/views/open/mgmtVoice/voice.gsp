<html>
<head>
    <title>Mgtvoice Content</title>
    <meta name="layout" content="moduleWebLayout"/>
</head>
<body>
<grailslab:breadCrumbActions breadCrumbTitleText="Voice and Managing Committee" SHOW_CREATE_BTN="YES" createButtonText="Add New"/>
    ...
    jQuery(function ($) {
        var validator = $('#modalForm').validate({
            ...
            submitHandler: function (form) {
                ...
                window.location.href = "${g.createLink(controller: 'mgmtVoice',action: 'create')}?featureType="+featureType;
            }
        });
        ...
        $('#list-table').dataTable({
            ...
            "sAjaxSource": "${g.createLink(controller: 'mgmtVoice',action: 'list')}",
        ...
        $('#list-table').on('click', 'a.reference-1', function (e) {
            ...
            window.location.href = "${g.createLink(controller: 'mgmtVoice',action: 'edit')}?id="+referenceId;
        });
        ...
        $('#list-table').on('click', 'a.reference-2', function (e) {
            ...
            if (confirmDel == true) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    dataType: 'JSON',
                    url: "${g.createLink(controller: 'mgmtVoice',action: 'inactive')}?id=" + referenceId,
        ...
        $('#list-table').on('click', 'a.reference-3', function (e) {
            ...
            if (confirmDel == true) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    dataType: 'JSON',
                    url: "${g.createLink(controller: 'mgmtVoice',action: 'delete')}?id=" + referenceId,
                    success: function (data, textStatus) {
                        if (data.isError == false) {
                            $('#list-table').DataTable().ajax.reload();
                            showSuccessMsg(data.message);
