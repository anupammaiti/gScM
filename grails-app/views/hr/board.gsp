...
<html>
<head>
    <title>Board Entry</title>
    <meta name="layout" content="moduleHRLayout"/>
</head>
<body>
<grailslab:breadCrumbActions breadCrumbTitleText="Add Board" SHOW_CREATE_BTN="YES" createButtonText="Add New Board"/>
<grailslab:fullModal label="Board Entry">
    ...
    jQuery(function ($) {
        var validator = $('#modalForm').validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    url: "${createLink(controller: 'hrSettings', action: 'boardSave')}",
                    type: 'post',
                    dataType: "json",
        ...
        $('#list-table').dataTable({
            ...
            "sAjaxSource": "${g.createLink(controller: 'hrSettings',action: 'boardList')}",
        ...
        $('#list-table').on('click', 'a.edit-reference', function (e) {
            ...
            jQuery.ajax({
                type: 'POST',
                dataType: 'JSON',
                url: "${g.createLink(controller: 'hrSettings',action: 'boardEdit')}?id=" + referenceId,
        ...
        $('#list-table').on('click', 'a.delete-reference', function (e) {
            ...
            if (confirmDel == true) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    dataType: 'JSON',
                    url: "${g.createLink(controller: 'hrSettings',action: 'boardDelete')}?id=" + referenceId,
                    success: function (data, textStatus) {
                        if (data.isError == false) {
        ...
        $('#list-table').on('click', 'a.inactive-reference', function (e) {
            ...
            if (confirmDel == true) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    dataType: 'JSON',
                    url: "${g.createLink(controller: 'hrSettings',action: 'boardInactive')}?id=" + referenceId,
                    success: function (data, textStatus) {
                        if (data.isError == false) {
