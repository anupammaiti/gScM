...
<body>
<grailslab:breadCrumbActions firstBreadCrumbText="Exam" firstBreadCrumbUrl="${g.createLink(controller: 'exam',action: 'index')}" secondBreadCrumbUrl="${g.createLink(controller: 'exam', action: 'classExams', id: shiftExamId)}" secondBreadCrumbText="Class List" breadCrumbTitleText="CT Schedule" SHOW_EXTRA_BTN1="${isEditSchedule?'YES':'NO'}" extraBtn1Text="Confirm Schedule and Publish"/>
<div class="row" id="create-form-holder">
    <div class="col-sm-12">
        <section class="panel">
            <header class="panel-heading">
                <span class="panel-header-title">Class: </span> <span class="panel-header-info">${className?.name}</span>  <span class="panel-header-title">Exam: </span> <span class="panel-header-info">${examName}</span><span class="panel-header-title">Schedule: </span> <span class="panel-header-info">CT Exam</span>
            </header>
                            ...
                            <table class="table table-striped table-hover table-bordered">
                                ...
                                <g:each in="${classSubjects}" var="classSubject" status="i">
                                    <tr>
                                        ...
                                        <td>
                                            <g:select class="form-control exam-period" value="${isEditSchedule ? classSubject?.ctPeriod : ''}" name="examPeriod${classSubject.subjectId}" tabindex="5"
                                                      noSelection="${['': 'Select Exam Time...']}"
                                                      from='${examPeriodList}'
                                                      optionKey="id" optionValue="name"></g:select>
                                        </td>
                                        <td><input type="text" name="examDate${classSubject.subjectId}" value="${isEditSchedule ? classSubject?.examDate : ''}"
    ...
    jQuery(function ($) {
        ...
        var validator = $('#create-form').validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    url: "${createLink(controller: 'examSchedule', action: 'saveClassCT')}?examIds=${examIds}&isNewSubject=${isNewSubject}",
                    type: 'post',
                    dataType: "json",
        ...
        $('.extra-btn-1').click(function (e) {
            ...
            if (confirmDel == true) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    dataType: 'JSON',
                    url: "${g.createLink(controller: 'examSchedule',action: 'publishCTSchedule')}/${shiftExamId}?className=${className.id}",
